...
export async function getDriverActiveOrder(driverId: number) { const db = await getDb(); if (!db) return undefined; const rows = await db.select().from(orders).where(and(eq(orders.driverId, driverId), inArray(orders.status, ["DRIVER_ASSIGNED", "DRIVER_GOING_TO_PICKUP", "ARRIVED_AT_PICKUP", "ORDER_PICKED_UP", "IN_DELIVERY", "ARRIVED_AT_DESTINATION"]))).orderBy(desc(orders.updatedAt)).limit(1); return rows[0]; }
